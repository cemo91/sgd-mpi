...
static void show_usage(string name){
    cerr << "Usage: " << name << " <option(s)>\n"
              ...
              << "\t-b,--batch-size BATCH_SIZE_PER_MACHINE: (int) Size of the batch for SGD updates\n"
...
int main(int argc, char* argv[]){
	...
	int success = process_file(data_m, file_name.c_str(), is_sparse, loss);
    if(success != 0){
        cerr << "File could not processed" << endl;
        return -1;
    }
	...
	gradient_check(data_m[generate_index(data_size)], loss, regularizer, lambda);

    Eigen::VectorXd weight(vector_size);
	
	double training_loss = train(data_m, weight, loss, learning_rate, step_size, regularizer, lambda, batch_size, max_epochs);
    
	cout << "------------------------------------------" << endl;
	cout << "Norm: " << weight.norm() << endl;
	cout << "------------------------------------------" << endl;
	cout << "Final training accuracy: " << compute_accuracy(data_m, weight, loss) << "%" << endl;
	cout << "------------------------------------------" << endl;
    cout << "Final loss: " << training_loss << endl;
    cout << "------------------------------------------" << endl;
